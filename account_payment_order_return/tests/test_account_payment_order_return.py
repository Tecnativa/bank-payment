    ...
    def test_global(self):
        ...
        self.payment_move = self.env["account.move"].create(
            self.invoice._reverse_move_vals(
                default_values={"journal_id": self.invoice.journal_id.id}
            )
        )
        ...
        with payment_return_form.line_ids.new() as line_form:
            line_form.move_line_ids.clear()
            line_form.move_line_ids.add(self.payment_line)
        ...
        self.payment_move.returned_payment = True
        wizard.populate()
