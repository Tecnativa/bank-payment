...
class account_invoice(orm.Model):
    ...
    def _get_move_line(self, cr, uid, invoice_id, context=None):
        return self.pool.get("account.move.line").search(
            cr,
            uid,
            [
    ...
    def _update_blocked(self, cr, uid, invoice, value, context=None):
        ...
        if invoice.move_id.id:
            move_line_ids = self._get_move_line(cr, uid, invoice.id, context=context)
            # work with account_constraints from OCA/AFT:
            context.update({"from_parent_object": True})
            self.pool.get("account.move.line").write(
                cr, uid, move_line_ids, {"blocked": value}, context=context
            )

    def _set_move_blocked(self, cr, uid, ids, name, field_value, arg, context=None):
        if context is None:
            context = {}
        if isinstance(ids, (int, long)):
            ids = [ids]
        for invoice in self.browse(cr, uid, ids, context=context):
            self._update_blocked(cr, uid, invoice, field_value, context=context)
    ...
    def action_move_create(self, cr, uid, ids, context=None):
        ...
        for invoice in self.browse(cr, uid, ids, context=context):
            self._update_blocked(
                cr, uid, invoice, invoice.draft_blocked, context=context
            )
        return res
