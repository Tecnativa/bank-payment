...
def create_simple_invoice(self, cr, uid, payment_term=False, context=None):
    ...
    return self.registry("account.invoice").create(
        cr,
        uid,
        {
...
class TestAccountBankingPaymentBlocking(common.TransactionCase):
    ...
    def test_invoice(self):
        ...
        invoice_id = create_simple_invoice(
            self, self.cr, self.uid, context=self.context
        )
        invoice_obj.write(self.cr, self.uid, [invoice_id], {"draft_blocked": True})
        workflow.trg_validate(
            self.uid, "account.invoice", invoice_id, "invoice_open", self.cr
        )
        invoice = invoice_obj.browse(
            self.cr, self.uid, [invoice_id], context=self.context
        )[0]
        ...
        self.assertEqual(
            invoice.blocked, move_line.blocked, "Blocked values are not equals"
        )
        move_line_obj.write(self.cr, self.uid, move_line_ids, {"blocked": False})
        invoice = invoice_obj.browse(
            self.cr, self.uid, [invoice_id], context=self.context
        )[0]
        move_line = move_line_obj.browse(self.cr, self.uid, move_line_ids)[0]
    ...
    def test_invoice_payment_term(self):
        ...
        self.assertEqual(invoice.blocked, False, "Blocked values are not equals")
